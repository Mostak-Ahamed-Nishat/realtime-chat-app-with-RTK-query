...
export default function Options({ info }) {
  ...
  const messageReceiver =info.receiver.email !== loggedInUserEmail.email?info.receiver:info.sender
  const messageSender =info.receiver.email === loggedInUserEmail.email?info.receiver:info.sender;

  //get the addConversation
  ...
  const handleSubmit = (e) => {
    ...
    editConversation({
      ...
      data: {
        participants: `${messageReceiver.email}-${messageSender.email}`,
        users: [messageReceiver, messageSender],
        message,
